...
export default function ClientResetPassword() {
  ...
  const recoveryData = useMemo(
    () => ({
      ...
      token:
        ...
        searchParams.get("tokenHash")?.trim() ??
  ...
  const submitForm = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!recoveryData.email || !recoveryData.token) {
      setFormError(t("authResetMissingToken"));
      return;
    }

    if (!password) {
      setFormError(t("authResetPasswordRequired"));
      return;
    }
    ...
    try {
      ...
      setFormError(getResetPasswordErrorMessage(error, t));
    } finally {
  ...
  return (
    <Page title={`Cliente - ${t("authResetPageTitle")}`}>
                ...
                <>
                  ...
                  <form onSubmit={submitForm} className="mt-5 grid gap-4">
                    ...
                    <Input
                      ...
                      label={t("authResetConfirmPassword")}
                      placeholder={t("authResetConfirmPasswordPlaceholder")}
                      type="password"
                      minLength={8}
                      prefix={<LockClosedIcon className="size-5" strokeWidth={1.5} />}
                    />
                    ...
                    >
                      {isSubmitting ? (
                        <AuthSubmittingLabel label={t("authResetSubmitting")} />
                      ) : (
                        t("authResetAction")
                      )}
                    </Button>
...
function getResetPasswordErrorMessage(
  ...
  return t("authResetGenericError");
}
